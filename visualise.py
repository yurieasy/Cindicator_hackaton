...
def plot_ticker(df):
    ...
    ax2.plot(df["y_baseline_max"], label='Baseline', drawstyle='steps')
    ax2.plot(df["y_true_max"], label='True', drawstyle='steps')
    ax2.plot(df["y_weighted_max"], label='Weighted', drawstyle='steps')
    ax2.legend()

    width = 0.35

    ax3.bar(x_axis, abs(df["y_baseline_min"]-df["y_true_min"]), width, label="Baseline error")
    ax3.bar(x_axis+width, abs(df["y_weighted_min"]-df["y_true_min"]), width, label="Weighted error")
    ax3.legend()

    ax4.bar(x_axis, abs(df["y_baseline_max"]-df["y_true_max"]), width, label="Baseline error")
    ax4.bar(x_axis+width, abs(df["y_weighted_max"]-df["y_true_max"]), width, label="Weighted error")
    ax4.legend()
